...
export function PasswordMatchValidator(ctrl_password_name: string, ctrl_confrim_password_name: string): ValidatorFn {
    return function (formGroup: AbstractControl): ValidationErrors | null {
        const ctrlPassword = formGroup.get(ctrl_password_name)
        const ctrlConfirmPassword = formGroup.get(ctrl_confrim_password_name)
